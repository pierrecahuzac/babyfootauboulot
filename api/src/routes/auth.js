...
import { createAuthMiddleware, getRateKey, isRateLimited, recordLoginAttempt, getIpKey, isGenericRateLimited, recordGenericAttempt, isRegisterRateLimited, recordRegisterAttempt } from '../middleware/auth.js';
...
export default async function authRoutes(app, { db, pool, users, players, matches }) {
  ...
  app.post('/api/auth/register', async (req, reply) => {
    ...
    // rate-limit après validation pour ne pas pénaliser les 400 (typos) + limite 20/15min par IP (bureau = IP partagée)
    const rateKeyReg = getIpKey(req, 'register');
    if (isRegisterRateLimited(rateKeyReg)) return reply.code(429).send({ error: 'trop de créations, réessaie dans 15 minutes' });
    recordRegisterAttempt(rateKeyReg);
