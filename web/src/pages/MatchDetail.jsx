        ...
        <div className="mt-5 grid grid-cols-3 items-center gap-2">
          <div className={`rounded-xl border p-3 text-center ${winBleue ? 'bg-sky-50 border-sky-200 dark:bg-sky-950/20' : 'bg-white dark:bg-zinc-800 border-zinc-200 dark:border-zinc-700'}`}>
            ...
            <div className="mt-2 space-y-1">
              {bleue.map((p, i) => (
                <p key={i} className={`text-sm font-medium ${p.deleted ? 'italic text-zinc-400 dark:text-zinc-500' : 'text-zinc-900 dark:text-zinc-100'}`}>{p.pseudo}{match.format !== '1v1' && p.poste ? <span className="text-xs text-zinc-500"> · {p.poste}</span> : null}</p>
              ))}
            </div>
