...
const Home = ({ players, onNav, user, league, ligue, onLeagues, onLigues, onRoadmap, onFeedback, onPlayerSelect }) => {
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="bg-white border border-zinc-200 dark:border-zinc-700 dark:bg-zinc-800 rounded-xl p-4">
          <div className="flex justify-between items-start gap-3">
            <div className="flex-1 min-w-0">
              <p className="font-semibold text-sm text-zinc-900 dark:text-zinc-100 truncate">{currentLeague.name}</p>
              <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">Ligue privée · {currentLeague.slug}</p>
              <div className="mt-2.5 flex items-center gap-2 text-xs flex-wrap">
                <span className="text-zinc-500">Code</span>
                {showCode ? (
                  <>
                    <span className="bg-zinc-900 dark:bg-zinc-700 text-white px-2.5 py-1 rounded-lg font-mono text-xs tracking-widest">{currentLeague.invite_code || currentLeague.inviteCode}</span>
                    <button onClick={() => { navigator.clipboard?.writeText(currentLeague.invite_code || currentLeague.inviteCode); }} className="border border-zinc-200 dark:border-zinc-600 px-2.5 py-1 rounded-lg font-medium bg-white dark:bg-zinc-700 hover:bg-zinc-50">Copier</button>
            ...
            <button onClick={handleLeagues} className="border border-zinc-200 dark:border-zinc-700 px-3 py-1.5 rounded-lg text-xs font-medium hover:bg-zinc-50 dark:hover:bg-zinc-700 shrink-0">Changer</button>
          </div>
        </div>
      )}
      {user && !currentLeague && (
        <div className="bg-amber-50 dark:bg-amber-950/20 border border-amber-200 dark:border-amber-900 rounded-xl p-4 text-center">
          ...
          <button onClick={handleLeagues} className="mt-3 bg-violet-600 text-white px-4 py-2 rounded-full text-sm font-medium">Gérer mes ligues</button>
        </div>
      )}
      ...
      {user ? (
        <div className="bg-white border border-zinc-200 dark:border-zinc-700 dark:bg-zinc-800 rounded-xl p-5">
          ...
          <div className="grid grid-cols-2 gap-2.5 mt-4">
            <button onClick={() => onNav('createMatch')} className="bg-violet-600 text-white py-3 rounded-xl font-medium text-sm hover:bg-violet-700">
              Créer un match
            </button>
            <button onClick={() => onNav('leaderboard')} className="border border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 py-3 rounded-xl font-medium text-sm hover:bg-zinc-50 dark:hover:bg-zinc-700">
              Classement
            </button>
          </div>
        </div>
      ...
      {user && (
        <button onClick={() => onNav('createMatch')} className="w-full bg-zinc-900 dark:bg-white dark:text-zinc-900 text-white py-3 rounded-xl font-medium text-sm">Nouveau match — {user.pseudo}</button>
      )}
