# État des lieux — Babyfoot — 2026-09-05 (dev:P0)
...
- **Front app** : React Vite Tailwind, `web/src/App.jsx` router + `12` pages `web/src/pages/*`, `utils/auth` `authFetch` `credentials:'include'` + `localStorage` fallback `babyfoot_token` + `babyfoot_theme` `🌙/☀️`, token URL purgé `history.replaceState` (`App.jsx`/`VerifyEmail.jsx:8`/`Reset.jsx:8`), CSP meta `web/index.html:4`. `Admin` modération + `demo`/`admin`.
