...
## ✅ Fait
- **Infra** : `docker-compose.yml` 4 services `db:5432` (healthy), `api:33333` (`PORT`, `JWT_SECRET`, volumes HMR), `web:55174` (`VITE_API_URL=""`, proxy `/api→api:33333`, `host 0.0.0.0`, `usePolling`), `landing:55175` (Astro). `0.0.0.0` pour réseau local. Plus de reverse proxy local, `Caddyfile.example` prêt pour prod `babyfootauboulot.dev` / `app.babyfootauboulot.dev`.
...
- Auth : pas de vérif email, pas de reset password, pas d'OAuth, JWT stocké en `localStorage` vs `httpOnly` cookie.
...
# app      http://localhost:55174 → app.babyfootauboulot.dev (ou réseau local)
